    ...
    fields: [
      {
        ...
        type: "llm_preset",
        defaultValue: "",
        help: "留空表示跟随当前聊天模型；选中已保存模板后，这个任务会独立使用对应模型配置。",
      },
    ],
  },
  {
...
const CODEX_TAB_META = {
  briefing: {
    kicker: "Codex 现场幕布",
    title: "幕布",
    desc: "时间、地点、天气、全球要闻、当地要闻和世界信号。",
  },
  world: {
    kicker: "Codex 世界引擎",
    title: "世界",
    desc: "世界状态、存档点、阵营、事件、风向和 Codex 回灌结果。",
  },
  aux: {
    kicker: "Codex 辅助桥",
    title: "辅助桥",
    desc: "格式修复、世界推衍、资料补全、现场幕布任务和最小上下文导出。",
  },
};
const CODEX_TAB_IDS = new Set(Object.keys(CODEX_TAB_META));
const CODEX_MOMERY_FUSION_KEY = "codex_momery_fusion_records_v1";
const CODEX_MOMERY_FUSION_LIMIT = 120;
const codexMountedPages = new Set();
...
function _isCodexTab(tabId = currentTabId) {
  return CODEX_TAB_IDS.has(tabId);
}

function _hashCodexString(value = "") {
  let hash = 2166136261;
...
function _compactCodexPayload(type, payload) {
  if (!payload || typeof payload !== "object") return payload ?? null;
  if (type === "world" && payload.state && typeof payload.state === "object") {
    ...
    return {
      ...
      events: Array.isArray(state.events) ? state.events.slice(0, 12) : [],
      factions: Array.isArray(state.factions) ? state.factions.slice(0, 12) : [],
      winds: Array.isArray(state.winds) ? state.winds.slice(0, 8) : [],
      memories: Array.isArray(state.memories) ? state.memories.slice(0, 12) : [],
    };
  }
  if (type === "briefing") {
    return {
      updatedAt: payload.updatedAt || payload.generatedAt || new Date().toISOString(),
      scene: payload.scene || payload.timePlace || {},
      weather: payload.weather || {},
      news: {
        global: Array.isArray(payload.news?.global) ? payload.news.global.slice(0, 6) : [],
        local: Array.isArray(payload.news?.local) ? payload.news.local.slice(0, 6) : [],
      },
      world: payload.worldSupport || payload.world || {},
...
function _summarizeCodexMemory(type, payload) {
  const data = payload && typeof payload === "object" ? payload : {};
  if (type === "briefing") {
    const scene = data.scene || {};
    return {
      title: "现场幕布",
      summary: [scene.timeText || scene.localTime || scene.dateText, scene.locationName || scene.location || scene.place]
        .filter(Boolean)
        .join(" · ") || "幕布数据已更新",
    };
  }
  if (type === "world") {
    return {
      title: "世界推衍",
      summary: data.worldDigest || `回合 ${Number(data.round || 0)}`,
    };
  }
  if (type === "aux-task") {
    return {
      title: "辅助桥任务",
      summary: data.taskFile || data.taskId || "任务快照已导出",
    };
  }
  if (type === "world-status") {
    return {
      title: "世界状态",
      summary: data.text || "状态已更新",
    };
  }
  return {
    title: type || "Codex",
...
function getCodexMemoryRecords() {
  try {
    const raw = localStorage.getItem(CODEX_MOMERY_FUSION_KEY);
...
function _saveCodexMemoryRecords(records = []) {
  localStorage.setItem(
    CODEX_MOMERY_FUSION_KEY,
    JSON.stringify(records.slice(0, CODEX_MOMERY_FUSION_LIMIT)),
  );
}

function addCodexMemoryRecord(type, payload = {}) {
  if (!type || payload == null) return getCodexMemoryRecords();
  const compact = _compactCodexPayload(type, payload);
  const digest = _hashCodexString(`${type}:${_safeCodexJson(compact).slice(0, 24000)}`);
  const existing = getCodexMemoryRecords().filter((record) => record.digest !== digest);
  const summary = _summarizeCodexMemory(type, compact);
  const record = {
    id: `${Date.now()}-${digest}`,
    digest,
    type,
  ...
  const next = [record, ...existing].slice(0, CODEX_MOMERY_FUSION_LIMIT);
  _saveCodexMemoryRecords(next);
  _renderCodexMemoryStrip();
  return next;
}

function _renderCodexMemoryStrip() {
  const strip = document.getElementById("bme-codex-memory-strip");
  if (!strip) return;
  const records = getCodexMemoryRecords().slice(0, 8);
  strip.replaceChildren();
  if (!records.length) {
    const empty = document.createElement("span");
    empty.className = "bme-codex-memory-empty";
    empty.textContent = "暂无融合记忆";
    strip.appendChild(empty);
    return;
  }
  for (const record of records) {
    const item = document.createElement("div");
    item.className = "bme-codex-memory-item";
    const title = document.createElement("b");
    title.textContent = record.title || record.type || "Codex";
    const summary = document.createElement("span");
    summary.textContent = record.summary || "已更新";
    item.append(title, summary);
    strip.appendChild(item);
  }
}

function _showCodexFallback(host, tabId, message = "等待扩展加载") {
  if (!host) return;
  host.replaceChildren();
  const box = document.createElement("div");
  box.className = "bme-codex-empty";
  const icon = document.createElement("i");
  icon.className = "fa-solid fa-circle-info";
  const text = document.createElement("div");
  const title = document.createElement("strong");
  title.textContent = CODEX_TAB_META[tabId]?.title || "Codex";
...
function _getCodexMountApi(tabId) {
  if (tabId === "briefing") return window.CODEX_BRIEFING_BOARD?.mountEmbedded;
  if (tabId === "world") return window.CODEX_WORLD_ENGINE_UI?.mountEmbedded;
  if (tabId === "aux") return window.CODEX_AUX_BRIDGE?.mountEmbedded;
  return null;
}

function _mountCodexPage(tabId) {
  const host = document.getElementById(`bme-codex-${tabId}-host`);
  if (!host) return;
  if (codexMountedPages.has(tabId)) return;
  const mount = _getCodexMountApi(tabId);
  if (!mount) {
    _showCodexFallback(host, tabId, "对应扩展还未加载，稍后会自动重试。");
    setTimeout(() => {
      if (currentTabId === tabId && !codexMountedPages.has(tabId)) _mountCodexPage(tabId);
    }, 1200);
    return;
  }
  try {
    ...
    Promise.resolve(result).then(() => {
      codexMountedPages.add(tabId);
      _renderCodexMemoryStrip();
    }).catch((err) => {
      console.warn("[Codex-Momery] embedded mount failed:", tabId, err);
      _showCodexFallback(host, tabId, err?.message || "嵌入页面加载失败");
    });
  } catch (err) {
    console.warn("[Codex-Momery] embedded mount failed:", tabId, err);
    _showCodexFallback(host, tabId, err?.message || "嵌入页面加载失败");
...
function _refreshCodexWorkspace() {
  if (!_isCodexTab()) return;
  const meta = CODEX_TAB_META[currentTabId] || CODEX_TAB_META.briefing;
  const kicker = document.getElementById("bme-codex-ws-kicker");
  const title = document.getElementById("bme-codex-ws-title");
  const desc = document.getElementById("bme-codex-ws-desc");
  if (kicker) kicker.textContent = meta.kicker;
  if (title) title.textContent = meta.title;
  if (desc) desc.textContent = meta.desc;
  document.querySelectorAll(".bme-codex-host").forEach((host) => {
    host.classList.toggle("active", host.dataset.codexPage === currentTabId);
  });
  _mountCodexPage(currentTabId);
  _renderCodexMemoryStrip();
}

function _refreshCurrentCodexModule() {
  if (currentTabId === "briefing") {
    window.CODEX_BRIEFING_BOARD?.refresh?.(true);
  } else if (currentTabId === "world") {
    window.CODEX_WORLD_ENGINE_UI?.refresh?.();
    try {
      addCodexMemoryRecord("world", {
        updatedAt: new Date().toISOString(),
        state: window.CODEX_WORLD_ENGINE_CORE?.loadState?.(),
      });
    } catch (_) {}
  } else if (currentTabId === "aux") {
    window.CODEX_AUX_BRIDGE?.exportTaskSnapshot?.();
  }
  _refreshCodexWorkspace();
}

function _bindCodexWorkspace() {
  const refreshBtn = document.getElementById("bme-codex-refresh");
  if (refreshBtn && refreshBtn.dataset.bmeCodexBound !== "true") {
    refreshBtn.addEventListener("click", () => _refreshCurrentCodexModule());
    refreshBtn.dataset.bmeCodexBound = "true";
  }
  const homeBtn = document.getElementById("bme-brand-home-btn");
  if (homeBtn && homeBtn.dataset.bmeHomeBound !== "true") {
    homeBtn.addEventListener("click", () => _switchTab("config"));
    homeBtn.dataset.bmeHomeBound = "true";
  }
}

function _installCodexMomeryApi() {
  window.CODEX_MOMERY = {
    ...(window.CODEX_MOMERY || {}),
    addCodexMemoryRecord,
    getCodexMemoryRecords,
    openCodexPage(page = "briefing") {
      if (!CODEX_TAB_IDS.has(page)) return false;
      openPanel();
      _switchTab(page);
      return true;
    },
...
function _removeLegacyApiConfigUi() {
  if (!panelEl) return;
  panelEl
    .querySelectorAll(
      '.bme-config-nav-btn[data-config-section="api"], .bme-config-section[data-config-section="api"]',
    )
    .forEach((element) => element.remove());
  if (currentConfigSectionId === "api") currentConfigSectionId = "toggles";
}
...
const FAB_VISIBLE_MIGRATION_KEY = "bme-fab-visible-migration";
const FAB_VISIBLE_MIGRATION_VERSION = "codex-momery-8.4.0";
let _fabEl = null;

function _startFabKeepAlive() {
  setInterval(() => {
    if (!_getFabVisible()) return;
...
function _getFabVisible() {
  try {
    if (localStorage.getItem(FAB_VISIBLE_MIGRATION_KEY) !== FAB_VISIBLE_MIGRATION_VERSION) {
      localStorage.setItem(FAB_VISIBLE_KEY, "false");
      localStorage.setItem(FAB_VISIBLE_MIGRATION_KEY, FAB_VISIBLE_MIGRATION_VERSION);
      return false;
    }
  } catch { /* ignore */ }
  try {
...
function _initFloatingBall() {
  ...
  // 应用可见性
  fab.style.display = _getFabVisible() ? "flex" : "none";
...
function _switchConfigSection(sectionId) {
  currentConfigSectionId = sectionId === "api" ? "toggles" : (sectionId || "toggles");
  _syncConfigSectionState();
...
const PANEL_VIEWPORT_KEY = "st-bme-panel-viewport";
const PANEL_VIEWPORT_DEFAULT = { scale: 1, offsetX: 0, offsetY: 0 };
const PANEL_SCALE_MIN = 0.7;
const PANEL_SCALE_MAX = 1.3;
const PANEL_SCALE_STEP = 0.1;
const PANEL_SIZE_STEP_W = 96;
const PANEL_SIZE_STEP_H = 64;
let _panelResizeTimer = null;
let _panelViewportDragState = null;

function _getPanelViewportSize() {
  return {
    h: window.visualViewport?.height || window.innerHeight || 720,
    w: window.visualViewport?.width || window.innerWidth || 1280,
  };
}

function _getPanelSizeBounds() {
  const viewport = _getPanelViewportSize();
  const maxW = Math.max(260, Math.round(viewport.w * 0.96));
  const maxH = Math.max(360, Math.round(viewport.h * 0.92));
  const minW = Math.min(maxW, Math.max(240, Math.round(viewport.w * 0.68)));
  const minH = Math.min(maxH, Math.max(320, Math.round(viewport.h * 0.56)));
  const defaultW = Math.min(maxW, Math.max(minW, Math.round(viewport.w * 0.82)));
  const defaultH = Math.min(maxH, Math.max(minH, Math.round(viewport.h * 0.72)));
  return { defaultH, defaultW, maxH, maxW, minH, minW };
}

function _normalizePanelSize(value = {}) {
  const bounds = _getPanelSizeBounds();
  return {
    h: Math.round(_clampPanelNumber(value.h, bounds.defaultH, bounds.minH, bounds.maxH)),
    w: Math.round(_clampPanelNumber(value.w, bounds.defaultW, bounds.minW, bounds.maxW)),
  };
}

function _savePanelSize(size) {
  const next = _normalizePanelSize(size);
  try {
...
function _syncPanelSizeControls(size = null) {
  const bounds = _getPanelSizeBounds();
  const next = _normalizePanelSize(size || {
    h: panelEl?.offsetHeight || bounds.defaultH,
    w: panelEl?.offsetWidth || bounds.defaultW,
  });
  document.getElementById("bme-panel-size-down")?.toggleAttribute(
    "disabled",
    next.w <= bounds.minW + 1 && next.h <= bounds.minH + 1,
  );
  document.getElementById("bme-panel-size-up")?.toggleAttribute(
    "disabled",
    next.w >= bounds.maxW - 1 && next.h >= bounds.maxH - 1,
  );
}

function _applyPanelSize(size, { persist = false } = {}) {
  if (!panelEl) return;
  const next = _normalizePanelSize(size);
  panelEl.style.width = `${next.w}px`;
  panelEl.style.height = `${next.h}px`;
  _syncPanelSizeControls(next);
  if (persist) _savePanelSize(next);
}

function _readPanelPixelSize(property, fallback) {
  const inlineValue = Number.parseFloat(panelEl?.style?.[property] || "");
  if (Number.isFinite(inlineValue) && inlineValue > 0) return inlineValue;
  const computedValue = Number.parseFloat(globalThis.getComputedStyle?.(panelEl)?.[property] || "");
  if (Number.isFinite(computedValue) && computedValue > 0) return computedValue;
  return fallback;
}

function _readActivePanelSize() {
  const bounds = _getPanelSizeBounds();
  return _normalizePanelSize({
    h: _readPanelPixelSize("height", panelEl?.offsetHeight || bounds.defaultH),
    w: _readPanelPixelSize("width", panelEl?.offsetWidth || bounds.defaultW),
  });
}

function _adjustPanelSize(direction) {
  const current = _readActivePanelSize();
  _applyPanelSize({
    h: current.h + PANEL_SIZE_STEP_H * direction,
    w: current.w + PANEL_SIZE_STEP_W * direction,
  }, { persist: true });
}
...
function _restorePanelSize() {
  ...
  try {
    ...
    const looksLikeMobileSnapshot =
      viewportWidth >= 900 &&
      Number.isFinite(w) &&
      Number.isFinite(h) &&
      w <= 380 &&
      h >= Math.min(560, viewportHeight * 0.7);
    if (looksLikeMobileSnapshot) {
  ...
  _syncPanelSizeControls();
}

function _clampPanelNumber(value, fallback, min, max) {
  const number = Number(value);
  const base = Number.isFinite(number) ? number : fallback;
  return Math.min(Math.max(base, min), max);
}

function _getPanelOffsetLimit(axis) {
  const size = axis === "x"
    ? (window.visualViewport?.width || window.innerWidth || 0)
    : (window.visualViewport?.height || window.innerHeight || 0);
  return Math.max(120, Math.round(size * 0.45));
}

function _normalizePanelViewport(value = {}) {
  return {
    scale: _clampPanelNumber(value.scale, PANEL_VIEWPORT_DEFAULT.scale, PANEL_SCALE_MIN, PANEL_SCALE_MAX),
    offsetX: Math.round(_clampPanelNumber(value.offsetX, PANEL_VIEWPORT_DEFAULT.offsetX, -_getPanelOffsetLimit("x"), _getPanelOffsetLimit("x"))),
    offsetY: Math.round(_clampPanelNumber(value.offsetY, PANEL_VIEWPORT_DEFAULT.offsetY, -_getPanelOffsetLimit("y"), _getPanelOffsetLimit("y"))),
  };
}

function _readPanelViewport() {
  try {
    const raw = localStorage.getItem(PANEL_VIEWPORT_KEY);
    return _normalizePanelViewport(raw ? JSON.parse(raw) : PANEL_VIEWPORT_DEFAULT);
  } catch {
    return _normalizePanelViewport(PANEL_VIEWPORT_DEFAULT);
  }
}

function _savePanelViewport(viewport) {
  try {
    localStorage.setItem(PANEL_VIEWPORT_KEY, JSON.stringify(_normalizePanelViewport(viewport)));
  } catch { /* ignore */ }
}

function _applyPanelViewport(viewport = _readPanelViewport(), { persist = false } = {}) {
  if (!panelEl) return;
  const next = _normalizePanelViewport(viewport);
  panelEl.style.setProperty("--bme-panel-scale", String(next.scale));
  panelEl.style.setProperty("--bme-panel-offset-x", `${next.offsetX}px`);
  panelEl.style.setProperty("--bme-panel-offset-y", `${next.offsetY}px`);
  panelEl.style.left = `${next.offsetX}px`;
  panelEl.style.top = `${next.offsetY}px`;
  panelEl.style.zoom = String(next.scale);
  panelEl.style.transform = "none";
  const label = document.getElementById("bme-panel-zoom-label");
  if (label) label.textContent = `${Math.round(next.scale * 100)}%`;
  document.getElementById("bme-panel-zoom-out")?.toggleAttribute("disabled", next.scale <= PANEL_SCALE_MIN + 0.001);
  document.getElementById("bme-panel-zoom-in")?.toggleAttribute("disabled", next.scale >= PANEL_SCALE_MAX - 0.001);
  if (persist) _savePanelViewport(next);
}

function _setPanelScale(scale) {
  const current = _readPanelViewport();
  _applyPanelViewport({ ...current, scale }, { persist: true });
}

function _adjustPanelScale(delta) {
  _setPanelScale(_readPanelViewport().scale + delta);
}

function _resetPanelViewport() {
  try {
    localStorage.removeItem(PANEL_SIZE_KEY);
  } catch { /* ignore */ }
  if (panelEl) {
  ...
  _applyPanelViewport(PANEL_VIEWPORT_DEFAULT, { persist: true });
  _syncPanelSizeControls();
}

function _beginPanelViewportDrag(event) {
  if (!panelEl || !event.currentTarget) return;
  const target = event.target;
  if (target?.closest?.("button, input, textarea, select, a, label, [contenteditable='true']")) return;
  const current = _readPanelViewport();
  _panelViewportDragState = {
  ...
  panelEl.classList.add("is-moving");
  event.preventDefault();
  event.currentTarget.setPointerCapture?.(event.pointerId);
  document.addEventListener("pointermove", _movePanelViewportDrag);
  document.addEventListener("pointerup", _endPanelViewportDrag, { once: true });
  document.addEventListener("pointercancel", _endPanelViewportDrag, { once: true });
}

function _movePanelViewportDrag(event) {
  if (!_panelViewportDragState || event.pointerId !== _panelViewportDragState.pointerId) return;
  _applyPanelViewport({
    scale: _panelViewportDragState.scale,
    offsetX: _panelViewportDragState.startOffsetX + event.clientX - _panelViewportDragState.startX,
    offsetY: _panelViewportDragState.startOffsetY + event.clientY - _panelViewportDragState.startY,
  });
}

function _endPanelViewportDrag(event) {
  if (!_panelViewportDragState || event.pointerId !== _panelViewportDragState.pointerId) return;
  document.removeEventListener("pointermove", _movePanelViewportDrag);
  document.removeEventListener("pointerup", _endPanelViewportDrag);
  document.removeEventListener("pointercancel", _endPanelViewportDrag);
  const next = _readPanelViewport();
  const activeStyle = panelEl
    ? {
        scale: _panelViewportDragState.scale,
        offsetX: _panelViewportDragState.startOffsetX + event.clientX - _panelViewportDragState.startX,
        offsetY: _panelViewportDragState.startOffsetY + event.clientY - _panelViewportDragState.startY,
  ...
  _panelViewportDragState = null;
  _applyPanelViewport(activeStyle, { persist: true });
}

function _bindPanelViewportControls() {
  const header = panelEl?.querySelector(".bme-panel-header");
  if (header && header.dataset.bmeViewportDragBound !== "true") {
    header.addEventListener("pointerdown", _beginPanelViewportDrag);
    header.dataset.bmeViewportDragBound = "true";
  }
  const zoomOut = document.getElementById("bme-panel-zoom-out");
  if (zoomOut && zoomOut.dataset.bmeViewportBound !== "true") {
    zoomOut.addEventListener("click", () => _adjustPanelScale(-PANEL_SCALE_STEP));
    zoomOut.dataset.bmeViewportBound = "true";
  }
  const zoomIn = document.getElementById("bme-panel-zoom-in");
  if (zoomIn && zoomIn.dataset.bmeViewportBound !== "true") {
    zoomIn.addEventListener("click", () => _adjustPanelScale(PANEL_SCALE_STEP));
    zoomIn.dataset.bmeViewportBound = "true";
  }
  const reset = document.getElementById("bme-panel-viewport-reset");
  if (reset && reset.dataset.bmeViewportBound !== "true") {
    reset.addEventListener("click", _resetPanelViewport);
    reset.dataset.bmeViewportBound = "true";
  }
  const sizeDown = document.getElementById("bme-panel-size-down");
  if (sizeDown && sizeDown.dataset.bmeViewportBound !== "true") {
    sizeDown.addEventListener("click", () => _adjustPanelSize(-1));
    sizeDown.dataset.bmeViewportBound = "true";
  }
  const sizeUp = document.getElementById("bme-panel-size-up");
  if (sizeUp && sizeUp.dataset.bmeViewportBound !== "true") {
    sizeUp.addEventListener("click", () => _adjustPanelSize(1));
    sizeUp.dataset.bmeViewportBound = "true";
  }
  _applyPanelViewport();
  _syncPanelSizeControls();
}
