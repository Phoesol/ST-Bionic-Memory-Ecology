...
const EXTENSIONS_SETTINGS_ENTRY_ID = "st_bme_extension_container";
const LEGACY_PROVIDER_SELECTORS = [
  "#codex_briefing_board_settings",
  "#codex_aux_bridge_settings",
  "#codex_aux_bridge_settings_inline",
  ".extension_block[data-name=\"/codex-briefing-board\"]",
  ".extension_block[data-name=\"/codex-aux-bridge\"]",
  ".extension_block[data-name=\"/codex-world-engine\"]",
];
...
function hideLegacyProviderEntries(doc) {
  if (!doc?.querySelectorAll) return;
  for (const selector of LEGACY_PROVIDER_SELECTORS) {
...
function injectExtensionsSettingsEntry(runtime) {
  syncBridgeLocale(runtime);
  const doc = runtime.document;
  if (!doc) return false;
  hideLegacyProviderEntries(doc);
  const mount =
    doc.getElementById("extensions_settings") ||
    doc.querySelector(".extensions_block") ||
    doc.getElementById("rm_extensions_block");
  if (!mount) return false;

  let container = doc.getElementById(EXTENSIONS_SETTINGS_ENTRY_ID);
  if (!container) {
    container = doc.createElement("div");
    container.id = EXTENSIONS_SETTINGS_ENTRY_ID;
    mount.insertBefore(container, mount.firstChild || null);
  }
  renderExtensionsSettingsEntry(container);
  const openButton = container.querySelector(".bme-extension-settings-entry__open");
  if (openButton && openButton.dataset.bmeOpenBound !== "true") {
    bindOpenPanelClick(runtime, openButton);
    openButton.dataset.bmeOpenBound = "true";
  }
  hideLegacyProviderEntries(doc);
  return true;
}
...
function scheduleOptionsMenuInjection(runtime, attempt = 0) {
  ...
  try {
    ...
    injectExtensionsSettingsEntry(runtime);
    hideLegacyProviderEntries(runtime.document);
