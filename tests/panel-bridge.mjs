...
function buildDocument({ includeExtensionsMenu = true, includeExtensionsSettings = false } = {}) {
  ...
  if (includeExtensionsSettings) {
    appendElement(document, document.body, "div", { id: "extensions_settings" });
  }
  return document;
}
...
{
  const document = buildDocument({ includeExtensionsSettings: true });
  const runtime = buildRuntime(document);

  await initializePanelBridgeController(runtime);
  const settingsEntry = document.getElementById("st_bme_extension_container");

  assert.ok(settingsEntry, "extension settings entry is injected");
  assert.match(settingsEntry.innerHTML, /Codex-Momery/, "extension settings entry uses Codex-Momery label");
  assert.equal(
    document.getElementById("extensions_settings")?.children[0],
    settingsEntry,
    "extension settings entry is inserted at the top",
  );
}

{
  ...
  assert.match(optionsEntry.innerHTML, /Codex-Momery/, "legacy menu entry follows English locale");
  assert.match(wandEntry.innerHTML, /Codex-Momery/, "magic-wand entry follows English locale");
  assert.match(fab.innerHTML, /Codex-Momery/, "floating bootstrap follows English locale");

  runtime.updateSettings({ uiLocale: "zh-CN" });
  await initializePanelBridgeController(runtime);
  assert.match(optionsEntry.innerHTML, /Codex-Momery/, "legacy menu entry refreshes when locale changes");
  assert.match(wandEntry.innerHTML, /Codex-Momery/, "magic-wand entry refreshes when locale changes");
  assert.match(fab.innerHTML, /Codex-Momery/, "floating bootstrap refreshes when locale changes");
}
